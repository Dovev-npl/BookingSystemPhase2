...
/*function createResourceDescriptionInput(container){
  ...
  input.placeholder = "e.g., Room with projector";
  input.rows = 4;

  input.className = '
    mt-2 w-full rounded-2x1 border border-black/10 bg-white
    px-4 py-3 text-sm outline-none
    focus:border-brand-blue focus:ring-2 focus:ring-brandd-blue/30
    transition-all duration-200 ease-out
';
...
function isResourceDescriptionValid(value) {
  ...
  const charactersValid = allowedPattern.text(trimmed);
...
function updateCreateButtonState() {
  const nameValid = resourceNameInput && isResourceNameValid(resourceNameInput.value);
  const descriptionValid = resoruceDescriptionInput && isResourceDescriptionValid(resourceDescriptionInput.value);

  setButtonEnabled(createButton, !!(nameValid && descriptionValid));
}
...
function attachResourceDescriptionValid(input) {
  const update = () => {
    const raw = input.value;
...
resourceDescriptionInput = createResourceDescriptionInput(resourceDescriptionContainer);
